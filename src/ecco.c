...
static void initSN_Name__(sqlite3* db)
{
    ...
    char* insertFormat = "INSERT INTO %s (SN, NAME) "
                         "VALUES ('%s', '%s' );";
    char insertValuesSql[5 * SUPERWIDEWCHARSIZE], tempSql[WIDEWCHARSIZE];
...
{
    FILE *fin = fopen("ecco.sn_name", "r"),
         *fout = fopen("levelThree", "w");
    wchar_t *wstr = getWString(fin), *tempWstr = wstr, resultWstr[SUPERWIDEWCHARSIZE] = { 0 }, lineStr[WCHARSIZE];
    ...
    while ((tempWstr += ovector[1]) && (length = wcslen(tempWstr)) > 0) {
        regCount = pcrewch_exec(snReg, NULL, tempWstr, length, 0, 0, ovector, 10);
        if (regCount <= 0)
            break;
