...
{
    static wchar_t testStr[2048], tempStr[1024];
    swprintf(testStr, 20, L"\n%s\n", L"testBoard：");
    wchar_t* FENs[] = { L"rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR",
        L"5a3/4ak2r/6R2/8p/9/9/9/B4N2B/4K4/3c5",
        L"2b1kab2/4a4/4c4/9/9/3R5/9/1C7/4r4/2BK2B2",
        L"4k4/P3a4/5r3/8p/2b6/9/9/7R1/4K4/9" };
    for (int i = 0; i < sizeof(FENs); ++i) {
        swprintf(tempStr, 1024, L"第%d个FEN ===================================================\n", i);
        wcscat(testStr, tempStr);
        wchar_t* pieChars = getPieChars_F(FENs[i], wcslen(FENs[i]));
        swprintf(tempStr, 1024, L"=>     FEN:%s size:%d\n=>PieChars:%s size:%d\n",
        ...
        wchar_t FEN[SEATNUM] = {};
        swprintf(tempStr, 1024, L"=>PieChars:%s size:%d\n=>     FEN:%s size:%d\n",
            pieChars, wcslen(pieChars), getFEN(pieChars), sizeof(FEN));
        wcscat(testStr, tempStr);

        swprintf(tempStr, 1024, L"%s=>&board：%p\n", getBoardString(&board, 1024));
        wcscat(testStr, tempStr);
        ...
        for (int row = 0; row < BOARDROW; ++row)
            for (int col = 0; col < BOARDCOL; ++col) {
                const Piece* pie = board.piece[row][col];
                if (pie != NULL) {
                    swprintf(tempStr, 1024, L"=>%s %d%d moveSeats:【", getPieString(pie), row, col);
                    wcscat(testStr, tempStr);
                    Seat seat = { row, col };
                    Seat seats[BOARDROW + BOARDCOL] = {};
                    int count = moveSeats(seats, &board, &seat);
                    for (int i = 0; i < count; ++i) {
                        swprintf(tempStr, 1024, L" %d%d", seats[i].row, seats[i].col);
                        wcscat(testStr, tempStr);
                    }
                    swprintf(tempStr, 1024, L"】 count: %d\n", count);
                    wcscat(testStr, tempStr);
                }
            }
        //*/
        ...
        for (int bottom = 0; bottom < 2; ++bottom)
        {
            ...
            for (int color = 0; color < 2; ++color)
            {
                for (int index = 0; index < PIECENUM; ++index)
                {
                    ...
                    wprintf(L"=>%s putSeats:【", pieString);
                    for (int i = 0; i < count; ++i)
